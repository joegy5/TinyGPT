...
class SinusoidalPE(nn.Module):
    def __init__(self, seq_len, d_model):
        ...
        self.pos = torch.arange(seq_len).unsqueeze(1).expand(-1, d_model).unsqueeze(0) # (N, d_model)
        self.div = 10000 ** (2 * torch.arange(end=seq_len, step=2).unsqueeze(0).expand(seq_len, -1) / d_model) # (N, d_model / 2)
