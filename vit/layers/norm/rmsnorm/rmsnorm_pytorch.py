...
class RMSNorm(nn.Module):
    def __init__(self, num_patches, d_model, eps=1e-8):
        ...
        self.gamma = nn.Parameter(torch.ones(size=(1, 1, self.d_model)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--b_dim", type=int, default=7, help="batch dimension of the test input")
    parser.add_argument("--n_dim", type=int, default=6, help="patch dimension of the test input")
    parser.add_argument("--d_dim", type=int, default=15, help="embedding dimension of the test input")
    args = parser.parse_args()

    rms_norm = RMSNorm(
        num_patches=args.n_dim+1, 
        d_model=args.d_dim, 
    )
